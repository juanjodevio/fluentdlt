...
class TestData:
    ...
    # Users table data
    USERS = [
        {"id": 1, "name": "Alice", "email": "alice@example.com", "age": 28, "is_active": True},
        {"id": 2, "name": "Bob", "email": "bob@example.com", "age": 35, "is_active": True},
        {"id": 3, "name": "Charlie", "email": "charlie@example.com", "age": 42, "is_active": False},
        {"id": 4, "name": "Diana", "email": "diana@example.com", "age": 31, "is_active": True},
        {"id": 5, "name": "Eve", "email": "eve@example.com", "age": 29, "is_active": True},
    ]
    ...
    # Products table data
    PRODUCTS = [
        {"id": 1, "name": "Widget", "category": "Electronics", "price": 25.00, "stock": 100},
        {"id": 2, "name": "Gadget", "category": "Electronics", "price": 50.00, "stock": 50},
...
def validate_user_data(loaded_data: list[dict]) -> bool:
    ...
    """
    assert len(loaded_data) == TestData.TOTAL_USERS, f"Expected {TestData.TOTAL_USERS} users, got {len(loaded_data)}"
...
def validate_event_data(loaded_data: list[dict]) -> bool:
    ...
    Returns:
        True if data is valid, raises AssertionError otherwise.
    """
    assert len(loaded_data) == TestData.TOTAL_EVENTS, f"Expected {TestData.TOTAL_EVENTS} events, got {len(loaded_data)}"
...
def validate_product_data(loaded_data: list[dict]) -> bool:
    ...
    Returns:
        True if data is valid, raises AssertionError otherwise.
    """
    assert len(loaded_data) == TestData.TOTAL_PRODUCTS, f"Expected {TestData.TOTAL_PRODUCTS} products, got {len(loaded_data)}"
