...
Fluent() \
    .from_db("postgresql://user:pw@host/db") \
    .from_query("SELECT * FROM orders WHERE created_at >= now() - interval '1 day'", table_name="recent_orders") \
...
FluentDLT bridges readability and performance.
Instead of YAML or heavy orchestration, you write pipelines that read like intent:
...
⚙️ CLI Usage
fldt s3 s3://bucket/data/*.csv --destination redshift --credentials "redshift://user:pw@host/db"
fldt db_table "postgresql://user:pw@host/db" public.users --destination redshift --write-disposition merge

🧾 License

Licensed under the Apache License 2.0
© 2025 Juan Palomino M.
FluentDLT builds upon the DLT-Hub
 library (Apache 2.0).
